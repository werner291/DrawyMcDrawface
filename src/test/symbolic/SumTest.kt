...
class SumTest {
	...
	@Test
	fun getters() {
		assertEquals(ScalarC(1.0f), (ScalarC(1.0f) + ScalarC(3.0f)).terms[1])
		assertEquals(ScalarC(3.0f), (ScalarC(1.0f) + ScalarC(3.0f)).terms[0])
	}
	...
	fun testSubstitute() {
		...
		val b = (ScalarC(1.0) + ScalarC(3.5)) + ScalarC(4.5)

		assertEquals(
				4.5f,
				b.substitute<Number>((ScalarC(1.0) + ScalarC(3.5)), ScalarC(0.0)).eval(),
				0.01f
		)
	...
	fun testCollectTerms() {
		...
		assertEquals(ScalarC(3) * x, (x + x + x).collectTerms())
		assertEquals(0, (x - x).eval())
